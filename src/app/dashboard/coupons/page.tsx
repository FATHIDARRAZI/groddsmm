...
export default function CouponsPage() {
  ...
  const submitCode = () => {
    ...
    // Placeholder verification logic before we build the DB
    setStatus({ type: 'error', msg: 'الكوبون غير صالح أو انتهت صلاحيته' });
  };
  ...
  return (
    <div className="w-full animate-fade-in max-w-4xl mx-auto flex flex-col items-center justify-center min-h-[60vh] text-center">
      ...
      <p className="text-slate-400 text-sm md:text-base max-w-lg leading-relaxed mb-8">
        هل تمتلك كود خصم أو كوبون هدية من الإدارة؟ قم بإدخاله هنا وسيتم شحن محفظتك بالنقاط فوراً! يتم نشر الأكواد يومياً في قناة التليجرام الخاصة بنا.
      </p>
      ...
      <div className="w-full max-w-md bg-[#1C1C1E] border border-white/5 rounded-2xl p-6 shadow-xl relative z-20">
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full bg-[#0B0F19] border border-white/5 rounded-xl py-4 pr-12 pl-4 text-center dir-ltr text-white font-bold leading-loose tracking-[0.2em] placeholder-slate-600 focus:outline-none focus:border-[#FF8577]/50 focus:ring-1 focus:ring-[#FF8577] transition-all"
          />
        </div>

        {status && (
          <div className={`mb-4 text-sm font-bold animate-pulse ${status.type === 'error' ? 'text-red-500' : 'text-green-500'}`}>
            {status.msg}
          </div>
        )}
        ...
        <button 
          onClick={submitCode}
          className="w-full py-4 rounded-xl font-extrabold text-white text-lg bg-gradient-to-r from-[#FF8577] to-[#FF6B6B] hover:opacity-90 active:scale-[0.98] transition-all shadow-[0_4px_20px_rgba(255,133,119,0.4)] flex items-center justify-center gap-3"
        >
           <i className="fas fa-check-circle"></i> تفعيل الكوبون
        </button>
      </div>
